  ...
  implements MigrationInterface
{
  public async up(queryRunner: QueryRunner): Promise<void> {
    const tokenSymbols = Object.values(TOKEN).map((t) => {
      return t.symbol;
    });
    const chainNames = Object.values(CREATOR_CHAIN).map((c) => {
      return c.shortName;
    });
    ...
    if (creatorsWithoutTokens.length > 0) {
      const values = creatorsWithoutTokens
        .flatMap(({ id }) => {
          return tokenSymbols.map((sym) => {
            return `('${sym}', ${id})`;
          });
        })
        .join(', ');
    ...
    if (creatorsWithoutNetworks.length > 0) {
      const values = creatorsWithoutNetworks
        .flatMap(({ id }) => {
          return chainNames.map((name) => {
            return `('${name}', ${id})`;
          });
        })
        .join(', ');
