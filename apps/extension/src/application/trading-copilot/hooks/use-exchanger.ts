...
import { useSolanaWallet, useWallet } from 'shared/extension';
...
export const useSolanaExchanger= ({ wallet }: SolanaProperties) => {
  const { setWalletInfo } = useSolanaWallet();
  const copilotTransaction = useCopilotTransaction();
  const getQuoteMutation = useCommandMutation(GetQuoteCommand);
  ...
  const exchange = useCallback(
    ...
    [wallet, copilotTransaction, getQuoteMutation, setWalletInfo],
  );
