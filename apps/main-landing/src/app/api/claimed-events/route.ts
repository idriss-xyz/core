...
const STARTING_BLOCK = 25614734n;

const fetchClaimedEvents = async (fromBlock: bigint, toBlock: bigint): Promise<ClaimEvent[]> => {
   let events: ClaimEvent[] = [];

   while (fromBlock <= toBlock) {
      const endBlock = fromBlock + BLOCK_RANGE > toBlock ? toBlock : fromBlock + BLOCK_RANGE;
...
const loadExistingEvents = (): { events: ClaimEvent[]; lastProcessedBlock: bigint } => {
...
const deduplicateEvents = (events: ClaimEvent[]): ClaimEvent[] => {
   const uniqueEvents = new Map(events.map((event) => [event.to, event]));
...
export async function GET(): Promise<NextResponse<ApiResponse>> {
   try {
      ...
      const newEvents = await fetchClaimedEvents(lastProcessedBlock + 1n, latestBlock);

      if (newEvents.length > 0) {
         const updatedEvents = deduplicateEvents([...existingEvents, ...newEvents]);
