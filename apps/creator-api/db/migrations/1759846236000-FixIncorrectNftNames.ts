...
import { InventoryFormat } from '@aws-sdk/client-s3';
...
async function getNftName(
  ...
  try {
    const meta = (await fetch(uri).then((r) => r.json())) as NftMeta;
...
export class FixIncorrectNftNames1759846236000 implements MigrationInterface {
  public async up(q: QueryRunner): Promise<void> {
    ...
    for (const nft of nfts) {
      const clientInfo = clients.find((info) => info.name === nft.network);
